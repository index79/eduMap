...
import 'package:eduMap/providers/providers.dart';
...
class ChatPageState extends State<ChatPage> {
  ...
  String formatTimestamp(String timestamp) {
    DateTime t = DateTime.fromMillisecondsSinceEpoch(int.parse(timestamp));
    final formatter = DateFormat('a h:mm', 'ko_KR');
    return formatter.format(t);
  }

  Widget buildItem(BuildContext context, index, DocumentSnapshot? document) {
    if (document != null) {
        ...
        return Row(
          ...
          children: <Widget>[
            if (isLastMessageRight(index))
              Text(formatTimestamp(messageChat.timestamp),
                  style: textTimestamp),
            // SizedBox(width: 10),
            messageChat.type == TypeMessage.text
                // Text
                ? buildBubble(
                    context: context, me: true, content: messageChat.content)
...
Widget expandedMessage(String content) {
  return Scaffold(
    appBar: AppBar(
      title: Text(
        '전체보기',
    ...
    onTap: () {
      if (content.length > 100) {
        Navigator.push(context,
            MaterialPageRoute(builder: (context) => expandedMessage(content)));
      }
    },
    child: Bubble(
      style: BubbleStyle(
        nip: me ? BubbleNip.rightTop : BubbleNip.leftTop,
        color: me ? Colors.grey[200] : Colors.blue[100],
        margin: BubbleEdges.only(top: 8.0, left: 5.0),
        elevation: 3,
            ...
            children: <TextSpan>[
              ...
              if (content.length >= 500)
                TextSpan(
                  ...
                  // You can adjust the fontSize and color to your preference
                ),
            ],
          ),
        ),
...
final textTimestamp = TextStyle(color: ColorConstants.greyColor, fontSize: 12);
